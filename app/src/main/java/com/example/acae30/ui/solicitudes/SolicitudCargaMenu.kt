...
import com.example.acae30.ui.solicitudes.NuevaSolicitud
        ...
        val adapter = SolicitudAdapter(lista, this@SolicitudCargaMenu) { i ->
            ...
            if (item.estado.contains("EMITIDO")) {
                lifecycleScope.launch(Dispatchers.IO) {
                    val procesada = solicitudController.validarSolicitudProcesadaEnServidor(
                        this@SolicitudCargaMenu,
                        item.idServidor
                    )
                    if (procesada) {
                        withContext(Dispatchers.Main) {
                            Toast.makeText(
                                this@SolicitudCargaMenu,
                                "SOLICITUD YA HA SIDO PROCESADA, FAVOR ACTUALICE",
                                Toast.LENGTH_SHORT
                            )
                                .show()
                        }
