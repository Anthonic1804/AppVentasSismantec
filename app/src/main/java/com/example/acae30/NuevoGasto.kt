...
class NuevoGasto : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val formaPago = ArrayAdapter<String>(this@NuevoGasto, android.R.layout.simple_spinner_dropdown_item)
        ...
        if(funciones.isInternetAvailable(this@NuevoGasto)){
            if(binding.txtMonto.text!!.isEmpty() || binding.txtMonto.text.toString().toFloat() <= 0){
                withContext(Dispatchers.Main){
                    Toast.makeText(this@NuevoGasto, "Debe de ingresar un monto correcto", Toast.LENGTH_SHORT).show()
                }
            }else{
