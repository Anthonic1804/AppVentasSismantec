...
class ValidarHojaCarga : AppCompatActivity() {
                    ...
                    lifecycleScope.launch(Dispatchers.IO) {
                        ...
                        if(productoSinValidar > 0){
                            withContext(Dispatchers.Main){
                                Toast.makeText(this@ValidarHojaCarga, "HAY PRODUCTOS SIN VALIDAR", Toast.LENGTH_SHORT)
                                    .show()
                            }
                        }else{
                            ...
                            if(aceptada){
                                withContext(Dispatchers.Main){
                                    Toast.makeText(this@ValidarHojaCarga, "PRODUCTO VALIDADOS CORRECTAMENTE", Toast.LENGTH_SHORT)
                                        .show()
                                ...
                                withContext(Dispatchers.Main){
                                    Toast.makeText(this@ValidarHojaCarga, "ERROR DE CONEXION, INTENTE NUEVAMENTE", Toast.LENGTH_SHORT)
                                        .show()
                                }
                            }
                        }
    ...
    private fun armarLista(lista : ArrayList<InventarioHojaValidar>) {
        ...
        val adapter = ValidarHojaAdapter(lista){
                item, isChecked ->
