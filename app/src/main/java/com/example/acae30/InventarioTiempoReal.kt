...
class InventarioTiempoReal : AppCompatActivity() {
    ...
    private fun mostrarLista(list: List<InventarioTiempoRealModel>) {
        try {
            if (list.isNotEmpty()) {
                    ...
                    lifecycleScope.launch {
                        ...
                        if (busquedaProducto) {
                            ...
                            if(sinExistencias == 0 && existeniasProducto <= 0f){
                                Toast.makeText(this@InventarioTiempoReal, "NO SE PUEDEN AGREGAR PRODUCTOS SIN EXISTENCIAS", Toast.LENGTH_SHORT).show()
                            }else{

                                cargarDatosProducto(id)

                                val intento = Intent(this@InventarioTiempoReal, Producto_agregar::class.java)
